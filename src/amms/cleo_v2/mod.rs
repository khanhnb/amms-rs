...
use crate::{amms::uniswap_v2::UniswapV2Error, finish_progress, update_progress};
use alloy::{
    eips::BlockId,
    network::Network,
        ...
        P: Provider<N> + Clone,
    {
        ...
        while let Some(res) = futures_unordered.next().await {
            ...
            for (pool_data, pool_address) in return_data.iter().zip(group.iter()) {
                ...
                let AMM::CleoV2Pool(pool) = amm else {
                    // TODO:: We should never receive a non CleoV2Pool AMM here, we can handle this more gracefully in the future
...
impl AutomatedMarketMakerFactory for CleoV2Factory {
    type PoolVariant = CleoV2Pool;
...
impl DiscoverySync for CleoV2Factory {
    fn discover<N, P>(
        &self,
        to_block: BlockId,
        provider: P,
